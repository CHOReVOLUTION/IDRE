...
import eu.chorevolution.syncope.utility.publishservice.serviceinventory.model.EnactmentEngine;
...
public class ApacheSyncopeUtilities {
    ...
    public ApacheSyncopeUtilities(String restUrl, String username, String password, String domain) {
        ...
        WebClient.client(choreographyService).accept(MediaType.APPLICATION_JSON_TYPE).type(MediaType.APPLICATION_XML_TYPE);
    ...
    public void createService(Service service) {
        ...
        serviceAnyObjectTO.getPlainAttrs().add(new AttrTO.Builder().schema(SERVICE_LOCATION)
                .value(service.getLocation()).build());
        ...
        /* skip the setting of relations to roles
        for (ServiceRole serviceRole : service.getServiceRoles()) {
            serviceAnyObjectTO.getRelationships().add(new RelationshipTO.Builder().type(SERVICE_ROLE_RELATIONSHIP)
    ...
    public AnyObjectTO getServiceByName(String serviceName) {
        return aoservice.search(new AnyQuery.Builder().fiql(
                new AnyObjectFiqlSearchConditionBuilder(SERVICE_SCHEMA).is(ANY_OBJECT_TO_NAME_PROPERTY).equalTo(serviceName).query())
    ...
    public List<AnyObjectTO> getServicesByRoleName(String roleName){
        ...
        do {
            pageres = aoservice.search(
                    new AnyQuery.Builder().page(page).fiql(new AnyObjectFiqlSearchConditionBuilder(SERVICE_SCHEMA)
                            .inRelationships(role.getKey()).query()).build());
    ...
    public AnyObjectTO createRole(String roleName, String roleDescription) {
        ...
        serviceInventoryRole.getPlainAttrs().add(new AttrTO.Builder().schema(SERVICE_ROLE_DESCRIPTION).value(roleDescription).build());
    ...
    public AnyObjectTO getRoleByName(String roleName) {
        return aoservice.search(new AnyQuery.Builder().fiql(new AnyObjectFiqlSearchConditionBuilder(SERVICE_ROLE_SCHEMA).is(ANY_OBJECT_TO_NAME_PROPERTY).equalTo(roleName).query()).build()).getResult().get(0);
    }
    ...
    public List<GroupTO> getSecurityRolesByNameContains(String securityRoleName) {
        ...
        int page = 1;
        do {
            pageres = groupService.search(
                    new AnyQuery.Builder().page(page).fiql(new GroupFiqlSearchConditionBuilder()
                            .is(CHOREOGRAPHY_IS_CHOREOGRAPHY).nullValue().and().is(ANY_OBJECT_TO_NAME_PROPERTY).equalTo("*" + securityRoleName + "*").query()).build());
    ...
    public List<AnyObjectTO> getEnactmentEnginesByNameContains(String enactmentEngineName) {
        ...
        do {
            pageres = aoservice.search(
                    new AnyQuery.Builder().page(page).fiql(new AnyObjectFiqlSearchConditionBuilder(ENACTMENT_ENGINE_SCHEMA)
                            .is(ANY_OBJECT_TO_NAME_PROPERTY).equalTo("*" + enactmentEngineName + "*").query()).build());
    ...
    public void createChoreography(String choreographyName, InputStream choreographyDeployment, EnactmentEngine enactmentEngine) throws IOException {
    	String synthesisProcessorID = "467a4bfb-d9eb-42b1-bbe8-0246e665e47f";
    	ChoreographyTO choreographyToUpload = new ChoreographyTO();
      choreographyToUpload.setName(choreographyName);
      choreographyToUpload.setSynthesisProcessorKey(synthesisProcessorID);
      choreographyToUpload.setDescription("");
      choreographyToUpload.setChorspec(IOUtils.toByteArray(choreographyDeployment));

    	choreographyService.create(choreographyToUpload);

    }

    public GroupTO getChoreographyByName(String choreographyName){
    	return groupService.search(
                new AnyQuery.Builder().fiql(new GroupFiqlSearchConditionBuilder()
                        .is(CHOREOGRAPHY_IS_CHOREOGRAPHY).equalTo("true").and().is(ANY_OBJECT_TO_NAME_PROPERTY).equalTo(choreographyName).query()).build()).getResult().get(0);
    }
    ...
    public List<GroupTO> getChoreographiesByNameContains(String choreographyName) {
        ...
        do {
            pageres = groupService.search(
                    new AnyQuery.Builder().page(page).fiql(new GroupFiqlSearchConditionBuilder()
                            .is(CHOREOGRAPHY_IS_CHOREOGRAPHY).equalTo("true").and().is(ANY_OBJECT_TO_NAME_PROPERTY).equalTo("*" + choreographyName + "*").query()).build());
    ...
    private void updateServiceRelationships(AnyObjectTO serviceAnyObjectTO, List<ServiceRole> serviceRoles){
        ...
        for (ServiceRole serviceRole : serviceRoles) {
            anyObjectPatch.getRelationships().add(new RelationshipPatch.Builder().operation(PatchOperation.ADD_REPLACE)
                    .relationshipTO(new RelationshipTO.Builder()
                            .type(SERVICE_ROLE_RELATIONSHIP)
                            .right(SERVICE_ROLE_SCHEMA, serviceRole.getKey()).build()).build());
        }
